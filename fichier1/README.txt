...
-raphael.cc : It the main file that I used, it is used to do all the selection of events, to apply the differents region of signal. The main of the code is in the ../sonicScrewdriver/interface/BabyScrewdriver.h file. If you want to have information about the cut and the region go to the ./Selection/moriond.h file. And if you want to have information about the variables go to the ../common/Reader_CommonFormat_CommonBabies.h file. The output are the plot in the plotsTest/ folder and the cards with the information about the number of events per region. The cards could be used with other code such as resultsPlot.cc combined_comparaison.cc or Zbi_result.cc . I let the card folder just because it is usefull to have them when you want the cards for combine.
